---
title: "idealImmunoTP User's Guide"
author:
...
package: "`r BiocStyle::pkg_ver('idealImmunoTP')`"
abstract: >
  In the scope of differential expression analysis, we provide the R/Bioconductor 
  package `r Biocpkg("idealImmunoTP")`, which serves as a web application to allow 
  simultaneously for interactive and reproducible analysis. `r Biocpkg("idealImmunoTP")`
...
output: 
  BiocStyle::html_document:
    toc_float: true
vignette: >
  %\VignetteIndexEntry{idealImmunoTP User's Guide}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}  
  %\VignettePackage{idealImmunoTP}
...
<!-- **Package**: `r Rpackage("idealImmunoTP")` -->

<!-- **Authors**: `r packageDescription("idealImmunoTP")[["Author"]]` -->

<!-- **Version**: `r packageDescription("idealImmunoTP")$Version` -->

**Compiled date**: `r Sys.Date()`

**Last edited**: 2019-02-06

**License**: `r packageDescription("idealImmunoTP")[["License"]]`
...
`r Biocpkg("idealImmunoTP")` is an R package distributed as part of the [Bioconductor](http://bioconductor.org) project. 
...
Once `r Rpackage("idealImmunoTP")` is installed, it can be loaded by the following command.
...
and then launch the app with the `idealImmunoTP` function. 
...
- `idealImmunoTP(countmatrix = countmatrix, expdesign = expdesign)`, where instead of passing the defined `DESeqDataSet` object, its components are given, namely the count matrix (e.g. generated after a run of featureCounts or HTSeq-count) and a data frame with the experimental covariates. 
  The design formula can be constructed interactively at runtime
- `idealImmunoTP()`, where the count matrix and experimental design can simply be uploaded at runtime, where all the derived objects can be extracted and computed live. 
...
- `Exit idealImmunoTP & save` will close the application and store the content of the `input` and `values` reactive objects in a list of two elements in the `ideal_env` environment, respectively called `ideal_inputs_YYYYMMDD_HHMMSS` and `ideal_values_YYYYMMDD_HHMMSS`
...
idealImmunoTP(dds_obj = dds_airway)
# or also providing the results object
idealImmunoTP(dds_obj = dds_airway,res_obj = res_airway)
```
...
idealImmunoTP(redo_dds_airway,edger_resu)
# or ...
idealImmunoTP(redo_dds_airway,limma_resu)
```
...
myde_enhanced <- myde
myde_enhanced$id <- idealImmunoTP:::createLinkENS(myde_enhanced$id,species = "Homo_sapiens")
myde_enhanced$symbol <- idealImmunoTP:::createLinkGeneSymbol(myde_enhanced$symbol)
...
mygo_enhanced$category <- idealImmunoTP:::createLinkGO(mygo_enhanced$category)
...
```{r sepguess}
sepguesser(system.file("extdata/design_commas.txt",package = "idealImmunoTP"))
sepguesser(system.file("extdata/design_semicolons.txt",package = "idealImmunoTP"))
sepguesser(system.file("extdata/design_spaces.txt",package = "idealImmunoTP"))
anyfile <- system.file("extdata/design_tabs.txt",package = "idealImmunoTP") # we know it is going to be TAB
